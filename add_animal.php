<?php
    ...
    if ($_POST) {
        ...
        if (strlen($type) > 0 && strlen($breed) > 0 && $age > 0 && strlen($gender) > 0 && strlen($color) > 0 && strlen($description) > 0) {
            // Upload image
            $target_dir = "uploads/";
            ...
            if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
                $query = "INSERT INTO animals (type, breed, age, gender, color, description, image_path) VALUES (:type, :breed, :age, :gender, :color, :description, :image_path)";
